...
async function runJob(mode, label) {
  ...
  while (true) {
    ...
    render();
    $("status").textContent = s.job?.running ? { filters: "Running the filters…", pick: "Reading the listings and asking Jev to pick one…", message: "Messaging the seller…" }[s.job.phase] || label : "";
